...
public class FluentMap<K, V> extends HashMap<K, V> {
    ...
    public FluentMap<V, Set<K>> inverse() {
        return entrySet().stream()
                         .collect(groupingBy((Function<Entry<K, V>, V>) Entry::getValue,
                                             FluentMap::new,
                                             mapping((Function<Entry<K, V>, K>) Entry::getKey, toSet())));
    }

    /**
     * Returns a map that holds the reverse of this map. Unlike {@link #inverse()} this method is lossy, meaning that any value which is pointed to by more than one key will only point back to one of those keys.
     *
     * <p><strong>NB:</strong> In case of collision, handling is non-deterministic.</p>
     */
    public FluentMap<V, K> inverseLossy() {
        return entrySet().stream().collect(toMap(Entry::getValue, Entry::getKey, (a, b) -> b, (Supplier<FluentMap<V, K>>) FluentMap::new));
    }
